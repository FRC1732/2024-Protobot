...
public class Robot extends LoggedRobot {
  ...
  private void setupNetworkPortForwarding() {
    String llName = VisionConstants.LIMELIGHT_NAME + ".local";

    for (int port = 5800; port <= 5807; port++) {
      PortForwarder.add(port, llName, port);
    }

    // For the second limelight, we need to shift the incoming ports by an
    // increment, say +10
    // PortForwarder.add(port+10, llName2, port);
  }
